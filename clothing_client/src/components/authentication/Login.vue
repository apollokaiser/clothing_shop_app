  ...
  <div class="login-container">
    <form class="form">
      ...
      <div class="inputForm">
        ...
        <input v-model="user.email" type="text" class="input" placeholder="Enter your Email">
      </div>
      <div class="flex-column">
      ...
      <div class="inputForm">
        ...
        <input v-model="user.password" type="password" class="input" placeholder="Enter your Password">
      ...
      <button @click.prevent="loginWithAccount" class="button-submit">Đăng nhập</button>
      <p class="p">Chưa có tài khoản?
...
const loginWithAccount = () => {
  auth.loginWithAccount(user).then(response => {
    if (response.status == 1003)
      loginFailed.message = "Tài khoản chưa được xác thực. Vui lòng vào hộp thư email để xác nhận";
    else if (response.status == 1005) {
      loginFailed.message = "Tài khoản đã được đăng ký thành viên !"
    } else
      loginFailed.message = "Tài khoản hoặc mật khẩu không đúng"
...
const loginWithGoogle = () => {
  googleSdkLoaded((google) => {
    google.accounts.oauth2.initCodeClient({
      client_id: '1000426255601-gegcrjo15vbu0vlaibgvlk43q2imtkk3.apps.googleusercontent.com',
      scope: 'email profile openid',
      callback: (response) => {
        auth.signInGoogle(response.code).then(res => res);
      }
    }).requestCode()
  })
}
