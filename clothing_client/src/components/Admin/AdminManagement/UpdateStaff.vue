...
import { updateStaff } from '@/data.function/postData';
...
onBeforeMount(() => {
    getStaffDetail(props.id).then(response=>{
        if(!response) {
            (async()=>{
                const {value:empty} = await Swal.fire({
                    title: 'Lỗi hệ thống',
                    text: 'Không tìm thấy nhân viên! Vui lòng thử lại',
                    icon: 'error',
                    cancelButtonText: 'Thoát ngay'
                })
                if (empty) {
                    router.back();
                }
            ...
            adminInfo.data = response;
        }
    })
})
const editStaff = () =>{
    if(adminInfo.data.tenNguoiDung =="" && adminInfo.data.email == "" && adminInfo.data.sdt=="") {
        alert("Vui lòng điền tất cả các trường dữ liệu !")
    } else {
        updateStaff(adminInfo.data, props.id).then(response =>{
            if(response?.status == 200) {
                ...
                Swal.fire("Cập nhật thất bại", "Cảnh báo", "error")
            }
        })
    }
}
        ...
        <div class="row tm-content-row">
            <div class="tm-block-col tm-col-account-settings">
                <div class="tm-bg-primary-dark tm-block tm-block-settings">
                    <h2 class="tm-block-title">Cập nhật nhân viên</h2>
                    <form action="" class="tm-signup-form row">
                        <div class="form-group col-lg-6">
                            <label for="name">Tên nhân viên</label>
                            <input v-model="adminInfo.data.tenNguoiDung" id="name" name="name" type="text"
                                class="form-control validate" required :placeholder="adminInfo.data.tenNguoiDung">
                        </div>
                        <div class="form-group col-lg-6">
                            <label for="email">Email</label>
                            <input v-model="adminInfo.data.adminEmail" id="email" name="email" type="email"
                                class="form-control validate" required :placeholder="adminInfo.data.adminEmail"/>
                        </div>
                        <div class="form-group col-lg-6">
                            <label for="sdt">Số điện thoại</label>
                            <input v-model="adminInfo.data.sdt" id="sdt" name="sdt" type="text"
                                class="form-control validate" required :placeholder="adminInfo.data.sdt"/>
                        </div>
                        <div class="form-group col-lg-6">
                            <label class="tm-hide-sm">&nbsp;</label>
                            <button @click.prevent="editStaff" type="submit"
